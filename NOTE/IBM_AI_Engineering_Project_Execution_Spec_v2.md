# IBM AI Engineering Capstone — Course Map + Project Execution Specification (v2)

> **Purpose:** A single execution/reference document for completing the IBM AI engineering capstone. It is designed for parallel use by **you** and a coding assistant such as Claude.
>
> **This version adds the complete LMS course structure visible in the Module 1–5 screenshots you provided.** The screenshots establish the full course progression, lesson titles, lab/checklist structure, the separate Final Project module, and the relationship between the learning modules and the final submission.

---

# 0. Course structure confirmed from the LMS screenshots

The screenshots show that the course contains **five modules**, followed by a course wrap-up. Modules 1–4 are the technical build sequence; Module 5 is the final project/submission and course wrap-up.

## Module 1 — Build a Structured Generative AI Application

**Module purpose:** Transform unstructured restaurant data into structured data, enrich it with multimodal information, and provide an interactive data-management interface.

### Lesson 1 — Structure Text Data with LLMs

Course items visible in the LMS:
...
- Predefined restaurant attributes
- Structured JSON output
- Prompt design
- JSON/schema validation
- Automatic repair of malformed LLM output
- Building the structured restaurant knowledge file

**Project source:** `M1L1.txt`
...
- Enriching structured JSON with generated visual descriptions

**Project source:** `M1L2.txt`
...
- Command-line restaurant management
- Browse/view restaurant records
- Add records
- Edit records
- Delete records
- LLM-powered structuring for new restaurant entries
- Confirmation before write operations
- Unit testing of interactive behavior
- Data persistence and integrity

**Project source:** `M1L3.txt`
...
**Module purpose:** Build the retrieval layer for the recommendation system using structured restaurant text and food images, then combine and rank results across modalities.

### Lesson 1 — Multimodal Vector Index Construction

Course items visible:
...
The supplied `M2L1.txt` specifies:

- Text model: `all-MiniLM-L6-v2`
- Text embedding dimension: **384**
- Image model: `CLIP (ViT-B/32)`
- Image embedding dimension: **512**
- Text collection: `restaurant_articles`
- Image collection: `food_images`
- L2 normalization for the embedding pipelines

**Project source:** `M2L1.txt`
...
Primary project topics visible from the course structure/final assignment:

- Similarity-based retrieval
- Retrieval query construction
- Top-K restaurant retrieval
- Metadata filtering
- Restaurant result inspection

**Important source note:** The detailed `M2L2.txt` reading was not among the uploaded text files. The LMS screenshot confirms that this lab exists, but the exact implementation instructions should be taken from the actual IBM lab when you open it rather than invented here.

### Lesson 3 — Multimodal Similarity Fusion and Ranking

Course items visible:

- Reading: **Assignment Overview: Multimodal Similarity Fusion and Ranking**
- Lab: **Multimodal Similarity Fusion and Ranking**
- Checklist: **Multimodal Similarity Fusion and Ranking**

Primary project topics visible from the course structure/final assignment:

- Multimodal retrieval results
- Late/weighted fusion
- Combining text and image similarity scores
- Ranking results under different weight configurations
- Final fused ranking output

**Important source note:** The detailed `M2L3.txt` reading was not among the uploaded text files. The LMS screenshot confirms that this lab exists, but the exact implementation instructions should be taken from the actual IBM lab when you open it.
...
**Module purpose:** Design specialized agents, orchestrate them into a recommendation workflow, test multi-agent collaboration, and expose the system through an interactive chatbot.
...
The supplied `M3L1.txt` defines six specialized agents:
...
The supplied `M3L2.txt` defines the intended workflow as a hybrid workflow:

```text
User analysis (sequential)
        ↓
Data retrieval (sequential)
        ↓
Analysis agents (parallel)
        ↓
Recommendation synthesis (sequential)
```

The three parallel analysis agents are:

- Food Trend Analyst
- Food Style Expert
- Nutrition Expert

The workflow is implemented with **LangGraph** concepts including:
...
Testing personas explicitly mentioned in the supplied material include:
...
The supplied `M3L3.txt` covers:

- Gradio chatbot interface
- Restaurant recommendation requests
...
- Add/update/delete database features

The implementation phases are described as:
...
**Module purpose:** Expose project data and functions through MCP, build the client side, and then build a full LLM-powered MCP host application.
...
The supplied `M4L1.txt` requires:

- MCP resource for the raw California Culinary Map text
- MCP tool for restaurant lookup by name
- MCP tool for vibe-based recommendation
- MCP tool for review retrieval
- Local FastMCP server execution
- Test client verification

Tools described:

- Restaurant lookup by name
- Recommendation by vibe
- Review retrieval

**Project source:** `M4L1.txt`
...
The supplied `M4L2.txt` requires:
...
- `list_roots` callback
- `handle_sampling` callback
- Shared tool-call helper
- Calls to all three server tools
...
The supplied `M4L3.txt` defines the final MCP application as:

- A Gradio-powered chat application
- An LLM connected to MCP server tools
- Runtime MCP tool discovery
- Tool-schema conversion for LLM tool binding
- ReAct agent loop
- Tool execution against the MCP server
- Iteration until a natural-language answer is produced
- Gradio Blocks UI
- Chat window
- Text input
- Quick-start prompt buttons
- Streaming/placeholder feedback

The target host is described as an application that lets a user ask natural-language questions about California restaurants and allows the LLM to discover and invoke the MCP tools.

**Project source:** `M4L3.txt`
...
**Module purpose:** Assemble and submit the required artifacts from the earlier labs and present the end-to-end AI engineering solution.
...
The supplied `Submission instruction.txt` states that the capstone is worth **60 points**, contains **12 tasks**, and requires screenshots corresponding to the work completed in the previous labs.

The screenshot-based submission covers:
...
# 0.1 LMS screenshot sources used for this revision

The five uploaded LMS screenshots were used only to expand the course-structure map in this version:

- `Module1.png` — Module 1 lesson structure and assessment
- `Module2.png` — Module 2 lesson structure and assessment
- `Module3.png` — Module 3 lesson structure and assessment
- `Module4.png` — Module 4 lesson structure and assessment
- `Module5.png` — Final Project and Course Wrap-Up structure

These screenshots confirm the existence and exact titles of the lessons, labs, checklists, role-play item, quizzes, and final-project submission area. They do **not** replace the individual lab instructions contained in the text files.

---

# 1. What the complete course is actually asking you to build

The LMS screenshots make the progression clearer than the individual reading files alone. The course is not six unrelated AI demos. It is a staged end-to-end system:

```text
MODULE 1
Structured Generative AI Application
        |
        v
Structured restaurant/customer data
        |
        v
MODULE 2
Multimodal RAG System
        |
        v
Vector indexes + retrieval + fusion/ranking
        |
        v
MODULE 3
Multi-Agent Recommendation System
        |
        v
Specialized agents + LangGraph workflow + chatbot
        |
        v
MODULE 4
Agents + RAG + Tools with MCP
        |
        v
MCP server + MCP client + LLM host
        |
        v
MODULE 5
...
This is the most useful mental model for implementation: **each module adds another layer to the same AI engineering system.**

---

# 2. Mandatory final evidence map

The final submission is evidence-driven. The LMS screenshots show that each earlier lab has a separate checklist and that Module 5 consolidates the resulting artifacts into the final graded submission.

| Final task | Build stage | Required screenshot |
|---|---|---|
| Q1 | M1L1 | `M1L1_structure_for_loop.jpg` |
| Q2 | M1L2 | `M1L2_caption_all_recipes.jpg` |
| Q3 | M1L3 | `M1L3_new_data_entry_process.jpg` |
| Q4 | M2L1 | `M2L1_multimodal_vector_index.jpg` |
| Q5 | M2L2 | `M2L2_similarity_retrieval.jpg` |
| Q6 | M2L3 | `M2L3_multimodal_fusion_results.jpg` |
| Q7 | M3L1 | `M3L1_food_style_expert_goal.jpg` |
| Q8 | M3L2 | `M3L2_node_analyze_styles.jpg` |
| Q9 | M3L3 | `M3L3_preference_extraction_test.jpg` |
| Q10 | M4L1 | `M4L1_Configure_Tools_Data_MCP_Server.jpg` |
| Q11 | M4L2 | `M4L2_Build_Test_MCP_Client.jpg` |
| Q12 | M4L3 | `M4L3_Design_LLM_MCP_Host.jpg` |

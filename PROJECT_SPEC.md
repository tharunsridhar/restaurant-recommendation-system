...
- MCP: pinned **`mcp<2`** in requirements.txt — the unpinned install grabbed `mcp==2.x`, which renamed `FastMCP` to `MCPServer`; the IBM lab text names "FastMCP" explicitly, and this repo's other proven MCP code (`9-Building AI Agents with MCP/`) targets 1.x, so pinned to match rather than adapt. The local MCP package also lives at **`src/mcp_app/`, not `src/mcp/`** — that name would shadow the real `mcp` SDK on `sys.path` (confirmed: it broke `from mcp.server.fastmcp import FastMCP`).
